...
class E2BEnvironment(BaseEnvironment):
    ...
    Commands have one deadline across dispatch and reconnection. On timeout or
    cancellation, Pier signals the wrapper to terminate the command's process
    group. Directory uploads preserve modes, symlinks, and empty directories.
    ...
    async def upload_file(self, source_path: Path | str, target_path: str):
        ...
        mode = stat.S_IMODE(Path(source_path).stat().st_mode) & 0o777
        result = await self.exec(
            f"chmod {mode:o} {shlex.quote(target_path)}", user="root"
        )
        if result.return_code != 0:
            raise RuntimeError(f"E2B file upload chmod failed: {result.stderr}")
    ...
    async def upload_dir(self, source_dir: Path | str, target_dir: str):
        ...
        # Like docker cp, transfer an archive so executable helpers, symlinks,
        # and empty directories survive. Keep large task trees off the heap.
        remote = f"/tmp/.pier-upload-{uuid.uuid4().hex}.tar"
        with tempfile.TemporaryDirectory(prefix="pier-e2b-upload-") as directory:
            ...
            try:
                ...
                result = await self.exec(
                    ...
                    f"tar --no-same-owner -xpf {shlex.quote(remote)} "
                    f"-C {shlex.quote(target_dir)}",
                    user="root",
                )
                if result.return_code != 0:
                    raise RuntimeError(f"E2B directory upload failed: {result.stderr}")
            finally:
                try:
                    await self._sandbox.files.remove(remote, user="root")
                except NotFoundException:
                    pass
                except Exception as exc:
                    self.logger.debug("Failed to remove E2B upload %s: %s", remote, exc)
    ...
    def _capture_command(command: str, capture_dir: str) -> str:
        ...
        return (
            ...
            # Bash monitor mode gives the command and its children a process
            # group. A TERM to this wrapper interrupts wait and kills the group.
            "__pier_pid=; "
            'trap \'if [ -n "$__pier_pid" ]; then '
            'kill -KILL -- -"$__pier_pid" 2>/dev/null || true; fi; exit 143\' '
            "TERM INT HUP; set -m; "
            f"( set +m; {command}\n) >{stdout_path} 2>{stderr_path} & "
            '__pier_pid=$!; wait "$__pier_pid"; '
    ...
    @retry(
        stop=stop_after_attempt(5),
        wait=wait_random_exponential(multiplier=1, max=10),
        retry=retry_if_exception_type(_API_RETRYABLE + _COMMAND_STREAM_RETRYABLE),
        reraise=True,
    )
    ...
    async def exec(
        ...
        dispatch = asyncio.create_task(
            self._dispatch_command(
                self._capture_command(
                    self._export_prefix(post_profile_env) + command, capture_dir
                ),
        ...
        handle = None
        try:
            async with asyncio.timeout(timeout_sec or None):
                # Cancellation must not lose a handle to a command that the
                # server has already started.
                handle = await asyncio.shield(dispatch)
                return await self._wait_command(handle, capture_dir)
        except BaseException:
            ...
            async def abort():
                ...
                try:
                    if handle is None:
                        handle = await asyncio.wait_for(dispatch, timeout=10)
                    await asyncio.wait_for(self._terminate_command(handle), timeout=10)
                except Exception as exc:
                    self.logger.warning("Failed to terminate E2B command: %s", exc)
                finally:
    ...
    async def _terminate_command(self, handle: AsyncCommandHandle) -> None:
        if self._sandbox is None:
            return
        # handle.kill() sends SIGKILL only to the wrapper PID, bypassing traps
        # and leaving its descendants alive. TERM lets the wrapper kill its job.
        await self._sandbox.commands.run(
            f"kill -TERM {int(handle.pid)} 2>/dev/null || true",
            user="root",
            cwd="/",
            timeout=5,
        )
        try:
            # The original stream may already be cancelled or disconnected.
            stopped = await self._sandbox.commands.connect(handle.pid, timeout=2)
            await asyncio.wait_for(stopped.wait(), timeout=2)
        except (CommandExitException, NotFoundException):
            pass
        except Exception:
            await handle.kill()
    ...
    async def _connect_command(self, pid: int) -> AsyncCommandHandle:
        if self._sandbox is None:
            raise RuntimeError("Sandbox stopped while command was running.")
        return await self._sandbox.commands.connect(pid, timeout=0)

    async def _wait_command(
        self, handle: AsyncCommandHandle, capture_dir: str
    ) -> ExecResult:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if self._sandbox is None:
                    raise RuntimeError("Sandbox stopped while command was running.")
                try:
                    handle = await self._connect_command(handle.pid)
                except NotFoundException:
                    captured = await self._read_command_capture(capture_dir)
                    if captured is None:
                        raise
