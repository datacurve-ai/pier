...
class GeminiCli(BaseInstalledAgent):
    ...
    def _load_gemini_session(self, path: Path) -> dict[str, Any] | None:
        ...
        for key, value in metadata.items():
            if key not in ("sessionId", "messages"):
                result[key] = value
        return result
    ...
    def _build_settings_config(
        ...
        if model and self._reasoning_effort:
            model_alias = f"pier-{model}-{self._reasoning_effort}"
            config["modelConfigs"] = {
                "customAliases": {
                    model_alias: {
                        "modelConfig": {
    ...
    async def run(
        ...
        env = self.build_process_env({"GEMINI_CLI_TRUST_WORKSPACE": "true"})
